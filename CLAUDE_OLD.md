...
The project uses a unified Cargo-orchestrated coverage system with language-specific tools:

#### Per-Language Coverage Generation

```bash
# 1. Rust Coverage (via cargo-llvm-cov)
cargo llvm-cov --workspace --lcov --output-path target/coverage/rust.lcov

# 2. C/C++ Coverage (via Clang + LLVM tools)
export LLVM_PROFILE_FILE="target/coverage/cpp-%p-%m.profraw"
CC=clang CXX=clang++ cargo test --features tracer_backend/coverage
llvm-profdata merge -sparse target/coverage/cpp-*.profraw -o target/coverage/cpp.profdata
llvm-cov export --format=lcov --instr-profile target/coverage/cpp.profdata \
  --object target/*/tracer_backend/test/* > target/coverage/cpp.lcov

# 3. Python Coverage (via pytest-cov + coverage-lcov)
cd query_engine && pytest --cov=query_engine --cov-report=xml:../target/coverage/python.xml
coverage-lcov -i ../target/coverage/python.xml -o ../target/coverage/python.lcov
```

#### Merge and Generate Unified HTML Report

```bash
# Merge all LCOV files
lcov -o target/coverage_report/merged.lcov \
  -a target/coverage/rust.lcov \
  -a target/coverage/cpp.lcov \
  -a target/coverage/python.lcov

# Generate unified HTML report
genhtml target/coverage_report/merged.lcov \
  --legend --branch-coverage \
  --output-directory target/coverage_report/html
```

#### Coverage Helper Tool

```bash
# Full coverage workflow (all languages → merged → HTML)
cargo run --manifest-path utils/coverage_helper/Cargo.toml -- full
```

#### Directory Structure
- `target/coverage/` - Temporary raw coverage data (.profraw, .profdata, per-language LCOV)
- `target/coverage_report/` - Persistent coverage reports
  - `merged.lcov` - Combined LCOV from all languages
  - `html/index.html` - Unified HTML report for human reading
  - `coverage_trend.csv` - Historical coverage tracking

#### Required Tools
- **Rust**: `cargo install cargo-llvm-cov`
- **C/C++**: Clang compiler + LLVM tools (llvm-cov, llvm-profdata)
- **Python**: `pip install pytest pytest-cov coverage-lcov`
- **Merging**: `brew install lcov` (provides lcov and genhtml)
...
- Rust – Use cargo-llvm-cov to generate LCOV format coverage
- C / C++ – **MANDATORY: Use Google Test (gtest/gmock) framework for all C/C++ tests**. Compile with Clang using coverage flags and use LLVM tools (llvm-profdata, llvm-cov) to generate LCOV format
- Python – Use pytest-cov with coverage-lcov to generate LCOV format
- Merging – Use lcov tool to merge all LCOV files and genhtml to create unified HTML reports
